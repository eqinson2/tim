...
public class PojoGenUtil {
    ...
    private static CtMethod generateClone(CtClass declaringClass, String className)
            throws CannotCompileException {
        String cloneBody = String.format("return (%s) super.clone();", className);
        String sb = String.format("public %s clone() { %s }", className, cloneBody);
        LOGGER.debug("generateToString:{}", sb);
        return CtMethod.make(sb, declaringClass);
    }
